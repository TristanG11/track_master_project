...
class Detect3DNode(LifecycleNode):
    ...
    def __init__(self) -> None:
        ...
        # parameters
        self.declare_parameter("target_frame", "left_color_frame")
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.depth_sub = message_filters.Subscriber(
            self, Image, "/apc/depth/image_raw", qos_profile=self.depth_image_qos_profile
        )
        self.depth_info_sub = message_filters.Subscriber(
            self, CameraInfo, "/apc/depth/camera_info", qos_profile=self.depth_info_qos_profile
        )
    ...
    def convert_bb_to_3d(
        ...
        # conversion aux dimensions image profondeur 
        h_color, w_color = 920, 1280
        h_depth, w_depth = 460, 640
        center_x = int(center_x * w_depth / w_color)
        center_y = int(center_y * h_depth /h_color)

        center_x = np.clip(center_x, 0, w_depth - 1)
        center_x = np.clip(center_y, 0, h_depth - 1)
        print(center_x)
