...
class YoloNode(LifecycleNode):
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self._sub = self.create_subscription(
            Image, "/apc/left/image_color", self.image_cb, self.image_qos_profile
        )
        ...
        for points in results.keypoints:
            ...
            for kp_id, (p, conf) in enumerate(zip(points.xy[0], points.conf[0])):
                ...
                if conf >= self.threshold:
                    ...
                    msg.id = kp_id + 1
                    msg.point.x = float(p[0]) * 1280 / target_width
                    msg.point.y = float(p[1]) * 920 / target_height 
    ...
    def image_cb(self, msg: Image) -> None:
        ...
        if self.enable:
            ...
            cv_image_resized = cv2.resize(cv_image, (target_width, target_height))
            self.get_logger().info(f"Taille de l'image{cv_image_resized.shape}")

            
            results = self.yolo.predict(
                source=cv_image_resized,
                verbose=False,
                stream=False,
                conf=0.3,
                iou=self.iou,
